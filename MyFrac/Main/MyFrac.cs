...
public class MyFrac
{
    ...
    {
        MyFrac result = new MyFrac(0, 1); //початкова сума (0/1)
        for (int i = 1; i <= n; i++)
        {
            //на кожній ітерації обчислюється новий дріб, де чисельник дорівнює 1, а знаменник — добуток і та і+1.
    ...
    {
        MyFrac result = new MyFrac(1, 1); //початковий добуток — одиниця(1/1)
        for (int i = 2; i <= n; i++)
        {
            //на кожній ітерації формується новий дріб factor, де чисельник дорівнює і+1, знаменник 2і
